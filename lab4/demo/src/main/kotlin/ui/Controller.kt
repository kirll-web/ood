...
class Controller(
    ...
    private val pictureDraft: PictureDraft
) {
    val consoleReader = DataInputStream(System.`in`)
    ...
    private fun parseCommand(command: String) {
        ...
        when (headCommand) {
            ...
            Commands.DRAW_PICTURE -> {pictureDraft.drawPicture(canvas)}

            Commands.CHANGE_SHAPE -> makeShapeAndApplyAction(
                args,
                pictureDraft,
                changeShapeAction = { id, strategy ->
                    pictureDraft.changeShape(id, strategy)
                }
            )
            ...
            Commands.CHANGE_COLOR -> {
                ...
                pictureDraft.changeColor(id, ShapeColor(colorString))
            }
            ...
            Commands.DRAW_SHAPE -> {
                ...
                pictureDraft.drawShape(id, canvas)
            }

            Commands.LIST -> pictureDraft.list()
            Commands.MOVE_PICTURE -> {
                ...
                pictureDraft.movePicture(dx.toDouble(), dy.toDouble())
            }
    ...
    private fun makeShapeAndApplyAction(
        ...
        addShapeAction: (PictureDraft.(shape: Shape) -> Unit)? = null,
        changeShapeAction: (PictureDraft.(id: String, strategy: IShapeTypeStrategy) -> Unit)? = null
    ) {
        ...
        getStrategy(shapeType, args)?.let { strategy ->
            ...
            changeShapeAction?.let { change ->
                getStrategy(shapeType, args)?.let {
                    strategy
                    pictureDraft.change(id, strategy)
                }
            }
        }
    }
    ...
    private fun moveShape(args: List<String>) {
        ...
        pictureDraft.moveShape(id, dx.toDouble(), dy.toDouble())
    }
